...
def gen1(stop):
   print('функ 1 сложение')
   n = 1
   while n <= stop:
       print('функ 1 число меньше заданного')
       yield n
       print('отработал генератор 1')
       n += 1
...
def gen2(start):
   ...
   n = start
   print('функ 2  число больше заданного')
   while n > 0:
       yield n
       print('отработал генератор 2')
...
def func(n):
   print("отработала функция запуска первых функций")
   yield from gen1(n)
   yield from gen2(n)
...
for n in func(56):
   print('отработала командна запуска функции запускающую функции')
   print(n, end=' ')
...
yield from помогает избавится от необходимости управлять перебором вручную. Иначе вам пришлось бы записать func(n) так:
...
Задача 2
Написать программу, которая считываем числа с консоли через запятую и выводит только числа кратные 3 или 5 через запятую в одну строку
